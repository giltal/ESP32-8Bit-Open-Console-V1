...

{
	...
	myCurrentFrameBuffer = frameBuffer;// new uInt8[160 * FRAME_END];
	myPreviousFrameBuffer = myCurrentFrameBuffer; //new uInt8[160 * 320];
...
bool TIA::saveDisplay(Serializer& out) const
{
	try
	{
		...
		out.putByteArray(myCurrentFrameBuffer, 160 * FRAME_END);
	}
	catch (...)
	{
...
bool TIA::loadDisplay(Serializer& in)
{
	try
	{
		...
		clearBuffers();
		myFramePointer = myCurrentFrameBuffer;
		in.getByteArray(myCurrentFrameBuffer, 160 * FRAME_END);
		memcpy(myPreviousFrameBuffer, myCurrentFrameBuffer, 160 * FRAME_END);
...
inline void TIA::startFrame()
{
	...
	// Stats counters
	myFrameCounter++;
	if (myScanlineCountForLastFrame >= FRAME_END)
		myPALFrameCounter++;
}
extern unsigned int renderCounter;
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
inline void TIA::endFrame()
{
	...
	if (myScanlineCountForLastFrame > myMaximumNumberOfScanlines + 1)
	{
		...
		{
			memset(myCurrentFrameBuffer, 0, 160 * FRAME_END);
			//memset(myPreviousFrameBuffer, 1, 160 * FRAME_END);
		}
	}
	...
	// If so, blank scanlines that weren't rendered this frame
		...
		myScanlineCountForLastFrame < FRAME_END && previousCount < FRAME_END)
	{
		...
		if ((myPreviousFrameBuffer + offset + stride) < (frameBuffer + 160 * FRAME_END))
		{
			memset(myCurrentFrameBuffer + offset, 0, stride);
	...
	{
		myFramerate = (myScanlineCountForLastFrame > FRAME_END ? 15600.0 : 15720.0) /
		...
		if (offset > myStopDisplayOffset && offset < 228 * 286)
			myStopDisplayOffset = offset;*/
		uInt32 offset = 160 * myScanlineCountForLastFrame;
		if (offset > myStopDisplayOffset && offset < 160 * FRAME_END)
			myStopDisplayOffset = offset;
	}
}
...
IRAM_ATTR void TIA::updateFrame(Int32 clock)
{
	...
	for (uInt32 line = startLine; line <= endLine; ++line)
	{
		...
		if (clocksToUpdate != 0 && (RenderFlag))
		{
			...
			if (ending > (frameBuffer + 160 * FRAME_END))
			{
				ending = frameBuffer + 160 * FRAME_END - 1;
			}
