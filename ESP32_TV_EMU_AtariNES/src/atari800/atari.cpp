...
static int load_roms(void)
{
    ...
    // Hardwire
    MEMORY_os = SYSROM_roms[Atari800_os_version].data;
    MEMORY_xe_bank = 0;
    //return TRUE;

#if 0
    // MEMORY_os is already selected, in rom
...
#ifdef TIGHT_MEM
        MEMORY_have_basic = basic_ver != -1 && SYSROM_LoadImage(basic_ver, (unsigned char*)MEMORY_basic);
        if (MEMORY_have_basic)
            MEMORY_basic = SYSROM_roms[basic_ver].data;
#else
        MEMORY_have_basic = basic_ver != -1 && SYSROM_roms[basic_ver].data;
        if (MEMORY_have_basic)
            memcpy(MEMORY_basic,SYSROM_roms[basic_ver].data,SYSROM_roms[basic_ver].size);
#endif
		if (!MEMORY_have_basic)
...
{
    if (!under_atarixl_os) {
        printf("under_atarixl_os on ESP will fail\n");
        //under_atarixl_os = Util_malloc(16*1024);
    }

    // dynamic on esp32
    if (!MEMORY_mem) {
        //MEMORY_mem  = Util_malloc(65536+2);
        //under_atarixl_os = Util_malloc(16*1024);
        //if (!MEMORY_mem || !under_atarixl_os)
        //    printf("MEMORY_mem/under_atarixl_os allocation in atari800 failed\n");
    }
#if !defined(BASIC) && !defined(CURSES_BASIC)
...
#if 1   // esptv don't use external roms/config
    got_config = TRUE;
#else
#ifndef ANDROID
