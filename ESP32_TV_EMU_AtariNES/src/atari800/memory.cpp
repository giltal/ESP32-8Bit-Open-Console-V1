...
#include "cpu.h"
#include "cartridge.h"
#include "roms_altirra_5200_os.h"
#include "esc.h"
#include "gtia.h"
#include "log.h"
#include "memory.h"
...
const UBYTE* MEMORY_os;         // can be rom TOODO

#ifdef TIGHT_MEM
const UBYTE* MEMORY_basic;      // ptr to rom: TODO make it loadable
UBYTE MEMORY_xegame[0];         // built in game, don't bother
#else
UBYTE MEMORY_basic[8192];       // can be rom TOODO
...
UBYTE* under_cart809F = 0;
UBYTE* under_cartA0BF = 0;
...
#else
static UBYTE antic_bank_under_selftest[0];
#endif
...
void MEMORY_StateRead(UBYTE SaveVerbose, UBYTE StateVersion)
{
	...
	if (Atari800_machine_type == Atari800_MACHINE_XLXE) {
		if (SaveVerbose)
			StateSav_ReadUBYTE((unsigned char*)&MEMORY_basic[0], 8192);
		StateSav_ReadUBYTE(&under_cartA0BF[0], 8192);

		if (SaveVerbose)
			StateSav_ReadUBYTE((unsigned char*)&MEMORY_os[0], 16384);
...
// track 32bit ram accesses on ESP32
void Map_memcpy(void* dst, const void* src, int len)
{
    //printf("Map_memcpy %08X %08X %04X\n",(uint32_t)dst,(uint32_t)src,len);
    memcpy(dst,src,len);
}
	...
	if ((oldval ^ byte) & 0x01) {
		if (byte & 0x01) {
			...
			if (MEMORY_ram_size > 48) {
				Map_memcpy(under_atarixl_os, MEMORY_mem + 0xc000, 0x1000);
				Map_memcpy(under_atarixl_os + 0x1800, MEMORY_mem + 0xd800, 0x2800);
			...
			/* Disable OS ROM */
			if (MEMORY_ram_size > 48) {
				Map_memcpy(MEMORY_mem + 0xc000, under_atarixl_os, 0x1000);
				Map_memcpy(MEMORY_mem + 0xd800, under_atarixl_os + 0x1800, 0x2800);
