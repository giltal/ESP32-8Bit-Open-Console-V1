...
#include "FS.h"
#include "SD.h"
static File disk[SIO_MAX_DRIVES];
#else
...
#ifdef USE_SD
#define Util_fopen(NAME, MODE, BLANK)		SD.open(NAME, MODE)
#define Util_fclose(FP, BLANK)				FP.close()
#define Util_flen(FP)						FP.size()
#define fclose(FP)							FP.close()
#define fread(BUF,BLANK,SIZE,FP)            FP.read((unsigned char *)BUF,SIZE)
#define fwrite(BUF,BLANK,SIZE,FP)			FP.write((const unsigned char *)BUF,SIZE)
#define fseek(FP, TO, BLANK)				FP.seek(TO)
#endif
int SIO_Mount(int diskno, const char *filename, int b_open_readonly)
{
	//printf("SIO Mount: %s %d\n", filename, b_open_readonly);
#ifdef USE_SD
	File f;
#else
	FILE *f = NULL;
	...
	if (!b_open_readonly)
	{
		f = Util_fopen(filename, "r+", sio_tmpbuf[diskno - 1]);		
		//f.seek(0);
	}
	if (!f)
	{
	...
	if (!b_open_readonly)
	{
		disk[diskno - 1] = Util_fopen(filename, "r+", sio_tmpbuf[diskno - 1]);
		//f.seek(0);
	}
	if (!disk[diskno - 1])
	{
